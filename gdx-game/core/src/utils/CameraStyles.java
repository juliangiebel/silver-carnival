...
public class CameraStyles {
	...
	public static void lerpToTarget(Camera camera,Vector2 target,float lerp)
	{
		...
		position.x = camera.position.x +( target.x*PPM - camera.position.x) * lerp;
		position.y = camera.position.y +( target.y*PPM - camera.position.y) * lerp;
		camera.position.set(position);
	...
	{
		Vector3 position = camera.position;
		position.x = (targetA.x + targetB.x )/2 *PPM;
		position.y = (targetA.y + targetB.y )/2 *PPM;
		camera.position.set(position);
	...
	{
		float avgx = (targetA.x + targetB.x )/2 *PPM;
		float avgy = (targetA.y + targetB.y )/2 *PPM;
		Vector3 position = camera.position;
